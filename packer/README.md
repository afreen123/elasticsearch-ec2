...
In future we can extend this code for cloud agnostic and add support for other clooud like  `azure`  or `gcp`.
Definitely, we need to update some configuration and Replace the `-only` parameter to `azure` to build images for Azure instead of AWS.
